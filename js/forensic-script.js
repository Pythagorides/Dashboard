...
function loadUPMCDashboards() {
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      //renderUPMCDashboards(upmcFiles);//Temporarily disabled to hide full dahsboard list
...
// Render all UPMC dashboards into the dynamic section
function renderUPMCDashboards(files) {
  const container = document.getElementById("upmc-dynamic");
  if (!container) return;
